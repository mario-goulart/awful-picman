...
(define (render-thumbnail thumb-obj id dimension prev-filename next-filename prev-id next-id mode)
...
(define (render-thumbnails items mode)
  (let* ((dim (default-thumbnail-dimension))
    ...
    `(div (@ (id "thumbnails"))
          ,@(append
             (map (lambda (thumb-obj)
                    `(div (@ (class "thumb"))
                          ,(case (thumb-type thumb-obj)
                             ((dir) (render-dir-link thumb-obj))
                             ((pic) (let* ((filename (thumb-filename thumb-obj))
                                           (dir (thumb-dir thumb-obj))
                                           (id (string->sha1sum filename))
                                           (prev (prev-thumb thumb-obj pics num-pics))
                                           (next (next-thumb thumb-obj pics num-pics))
                                           (prev-id (and prev (string->sha1sum prev)))
                                           (next-id (and next (string->sha1sum next))))
                                      (set! modals
                                            (cons (render-pic-modal dir id filename prev-id next-id)
                                                  modals))
                                      (render-thumbnail thumb-obj id dim prev next prev-id next-id mode)))
                             (else (render-other-file-type (thumb-filename thumb-obj))))))
                  items)
             modals))))
